...
def process_soft_sentiment_subset(df_subset: pd.DataFrame, sentiment_label: str, cfg: Dict[str, Any], forced_k: Optional[int] = None) -> pd.DataFrame:
    ...
    """
    # 1. Recuperamos la semilla aleatoria
    seed = cfg["project"]["random_state"]

    # 2. Parámetros base de features, con override por sentimiento si existe en el config
    ...
    # 3. Filtro de longitud mínima específico por sentimiento
    ...
    # 4. LDA requiere una matriz de conteos (Bag of Words), no TF-IDF.
    # Construimos el vectorizador con los parámetros específicos de este sentimiento.
    print(f"\n  [{sentiment_label.upper()}] Vectorizando con CountVectorizer...")
    print(f"  [{sentiment_label.upper()}] Parámetros: max_features={features_cfg['max_features']}, "
          f"min_df={features_cfg['min_df']}, max_df={features_cfg['max_df']}, "
          f"ngram_range={features_cfg.get('ngram_range', [1, 1])}")
    ...
    # 5. Ajustamos el modelo a los textos y obtenemos la matriz X, el modelo ajustado y el vocabulario
    ...
    # 6. Intentamos calcular la coherencia c_v usando Gensim.
    # Si no está disponible o el vocab es de bigramas puros, usamos Log-Likelihood como fallback.
    scores = None
    ...
    if scores is None:
        print(f"  [{sentiment_label.upper()}] Usando Log-Likelihood (modo bigrama o sin gensim)...")
    ...
    # 7. Determinamos el número de tópicos. Se prioriza el valor manual (forced_k) si existe.
    ...
    # 8. Exportamos el gráfico de línea de la métrica evaluada (Coherencia o Log-Likelihood)
    ...
    # 9. Entrenamos el modelo LDA final. Obtenemos la matriz documento-tópico y el tópico dominante.
    lda_model, doc_topic_matrix, dominant_topics = fit_lda_model(X, optimal_k, seed, cfg["model"]["final_max_iter"])

    # 10. Asignamos el tópico dominante al DataFrame y extraemos las palabras clave por grupo
    ...
    # 11. Muestreamos la matriz de distribución de tópicos para generar el mapa t-SNE
    X_sampled, labels_sampled = sample_data_for_tsne(
        doc_topic_matrix, dominant_topics, cfg["visualization"]["tsne"]["samples"], seed
    )

    # 12. Reducimos las dimensiones a 2D basándonos en las distancias de probabilidad entre tópicos
    coords_2d = reduce_dimensions_with_tsne(
        X_sampled, cfg["visualization"]["tsne"]["perplexity"], seed, cfg["visualization"]["tsne"]["max_iter"]
    )

    # 13. Guardamos la visualización estática t-SNE coloreada por el tópico de mayor peso
    ...
    # 14. Exportamos el panel interactivo HTML usando pyLDAvis
    ...
    # 15. Preparamos el DataFrame de salida con etiquetas de palabras clave y probabilidades por tópico
...
def run_soft_pipeline(config_path: str) -> None:
    ...
    # 3. Cargamos el CSV manejando automáticamente filas corruptas
    ...
    # 5. Limpieza de texto básica (minúsculas, puntuación, URLs, números)
    df['review_limpio'] = df[cfg["data"]["text_col"]].apply(apply_basic_text_cleaning)
    # Descomentar para activar lematización:
    # df['review_limpio'] = df['review_limpio'].apply(lemmatize_text)

    # 6. Segmentamos el dataset en grupos por sentimiento
    ...
    # 7. Pipeline LDA para reseñas positivas (con parámetros específicos si están definidos)
    ...
    # 8. Pipeline LDA para reseñas negativas (con K forzado y parámetros específicos si están definidos)
    ...
    # 9. Concatenamos y exportamos el CSV final
