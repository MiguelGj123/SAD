...
#TODO aqui lo que haga falta para pasar de categorial a numerico
    ...
    try:
        if len(categorical_feature.columns)>0 :
            ...
            # (Comentada para que elijas). Crea una columna nueva por cada categoría con 0s y 1s.
            ...
            # Para usar esta, Pandas tiene una función maravillosa que lo hace directo:
...
def over_under_sampling():
    ...
    global data
    try:
        estrategia = args.preprocesing.get("sampling", "none").lower()
        if estrategia == "none":
